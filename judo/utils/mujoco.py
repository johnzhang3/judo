...
class RolloutBackend:
    ...
    def __init__(
        self, num_threads: int, backend: Literal["mujoco", "cpp", "cpp_persistent", "onnx", "onnx_persistent"]
    ) -> None:
    ...
    def policy_rollout(
        ...
        """
        if self.backend not in ["onnx", "onnx_persistent"]:
            raise ValueError("Policy rollout only available for ONNX backends")
